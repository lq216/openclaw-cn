---
summary: "CLI reference for `openclaw-cn security` (audit and fix common security footguns)"
read_when:
...
```bash
openclaw-cn security audit --json | jq '.summary'
openclaw-cn security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
openclaw-cn security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
